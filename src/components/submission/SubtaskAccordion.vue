        ...
        <div class="accordion" :id="data_index" v-if="result.IsSubtask">
            <div class="accordion-item" v-for="(sub, index) in result.Subtask" :key="index">
                <h2 class="accordion-header">
                    <button :class="'accordion-button collapsed btn result-' + (isSubAC(sub) ? 'success' : 'failed')"
                        type="button" data-bs-toggle="collapse"
                        :data-bs-target="'#collapse_' + data_index + '_subtask' + index">
                        Subtask #{{ index }}: {{ getSubScore(sub) }}/{{ sub.Fullscore }}pts
                    </button>
                </h2>
                <div :id="'collapse_' + data_index + '_subtask' + index" class="accordion-collapse collapse"
                    :data-bs-parent="'#' + data_index">
